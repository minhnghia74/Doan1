...
{
    FILINFO fno;
    FRESULT res; // return result interface sd
...
static long WlanConnect()
{
    ...
    lRetVal = sl_WlanConnect((signed char *) SSID_NAME,
                             strlen((const char *) SSID_NAME), 0, &secParams,
                             0);
    ASSERT_ON_ERROR(lRetVal);
...
//!
//*****************************************************************************
static int HTTPPostMethod(HTTPCli_Handle httpClient, const char *post_data)
{
    bool moreFlags = 1;
    bool lastFlag = 1;
    int content_length;
    ...
    sprintf(content_length_header, "%d", content_length);

    lastFlag = 1;
    lRetVal = HTTPCli_sendField(httpClient, HTTPCli_FIELD_NAME_CONTENT_LENGTH,
                                (const char *) content_length_header, lastFlag);
    if (lRetVal < 0)
    {
        UART_PRINT("Failed to send HTTP POST request header.\n\r");
        return lRetVal;
    ...
    /* Send POST data/body */
    lRetVal = HTTPCli_sendRequestBody(httpClient, post_data, content_length);
    if (lRetVal < 0)
    {
...
void SPI_RFID()
{
    /// Master Mode to read data from RC522
    ...
    GPIOPinWrite(GPIOA0_BASE,0x40, 0); // CS LOW
    Init_MFRC522();

    unsigned char tmp = ReadReg(VersionReg);
}

char *stohex(const char *in, int length)
{
    int i = 0;
...
void sdcardfats_config()
{
    ...
    MAP_SDHostSetExpClk(SDHOST_BASE,
                            MAP_PRCMPeripheralClockGet(PRCM_SDHOST),8000000);
...

{
    ...
    // Thêm mode re-connect wifi
    // Thêm loại card
    // Thêm Id node
    // Interface cho người dùng
    // Thêm id của node vào database.
...
//    Lcd_clear();
    ...
    while (1)
    {
        ...
        status = Request(PICC_REQIDL, serNum); //serNum = cardType
        if (status == MI_OK){
            Message("\n Card Detected !!!! ");
...
//                break;
//            case 0x0400:
//                Lcd_Print("Mifare_One ");
//                break;
//            case 0x0200:
//                Lcd_Print("MifareOneS70");
//                break;
//            case 0x0800:
//                Lcd_Print("Mifare_Pro");
//                break;
//            case 0x4403:
//
//                Lcd_Print("MifareDESFire");
//                break;
//            default:
//                Lcd_Print("UnknownType");
//                break;
//            }
        }


        status = Anticoll(serNum); //serNum = card serial
        if(status == MI_OK)
        {
            char *hexserNum = stohex(serNum, 5);
            Report("\r\n CARD ID: %s", hexserNum);
            readfile();
           // goto xy;
...
//            lRetVal = HTTPPostMethod(&httpClient, hexserNum);
//            if (lRetVal < 0)
//                Message("\r\nFail to post data");
